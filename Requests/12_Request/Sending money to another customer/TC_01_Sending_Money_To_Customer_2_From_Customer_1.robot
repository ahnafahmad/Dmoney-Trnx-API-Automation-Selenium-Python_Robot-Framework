...
TC1: Deposit to Customer number
    ...
    ${customer_2_phone_number}=      get value from json    ${json_obj}      $.customer_2_phone_number
    ${customer_1_phone_number}=      get value from json    ${json_obj}      $.customer_1_phone_number
    ${amount}=      convert to integer    50
    ${user_info}=       create dictionary    from_account=${customer_1_phone_number[0]}        to_account=${customer_2_phone_number[0]}     amount=${amount}
    ...
    ${response}=        POST On Session    mysession        /transaction/sendmoney        data=${user_info_json}      headers=${header}
